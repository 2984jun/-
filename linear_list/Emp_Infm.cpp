...
void SaveFile(EmpList *L)
{
    ...
    if((fp=fopen("emp.txt","w"))==NULL)
    {
        printf("    提示:不能创建文件emp.txt\n");
        return;
    }
    while(p!=NULL)
    {
        fwrite(&p->data,sizeof(EmpList),1,fp);
        p=p->next;
        n++;
    }
...
int main(void)
{
    EmpList *L;
    int sel;
    printf("由emp.txt文件建立职工单链表L\n");
    ReadFile(L);
    do
    {
        printf(">1:添加 2:显示 3:按职工号排序 4:按部门号排序 5:按工资排序\n");
        printf(">6:删除 9:全删除 0:退出 请选择:");
        scanf("%d",&sel);
        switch(sel)
        {
            case 9:
